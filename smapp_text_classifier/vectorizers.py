...
class CachedVectorizer(object):
    ...
    def _load_from_cache(self):
        if self.recompute:
            raise CacheError()
        elif os.path.exists(self.cache):
            cached_self = joblib.load(self.cache)
...
class CachedCountVectorizer(CountVectorizer, CachedVectorizer):
    ...
    def fit_transform(self, X, y=None):
        try:
            ...
            self.feature_matrix = super().fit_transform(X)
...
class CachedEmbeddingVectorizer(TransformerMixin, BaseEstimator, 
    ...
    def transform(self, X, y=None):
        try:
            ...
            self.feature_matrix = np.array([self._embed_doc(doc) for doc in X])
            joblib.dump(self, self.cache)
            return self.feature_matrix
    ...
    def _embed_doc(self, doc):
        ...
        vectors = np.array([self._get_vector(t, em.wv) for t in tokens])
